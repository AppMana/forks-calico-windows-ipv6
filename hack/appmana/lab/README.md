...
go run . -case windows-tests -image YOUR_PRELOADED_WINDOWS_IMAGE \
  ...
  -binary /absolute/artifacts/calico-windows.test.exe -artifacts /absolute/evidence/windows
```
...
The test executable's SHA-256 is printed. Guests are destroyed when the runner
